...
def reset_new_0(env):
    ...
    print("00000000",env.task.target_pos)
    env.robots[0].robot_specific_reset()
    env.robots[0].set_position([100.0, 100.0, 100.0])
    

    initial_pos = env.task.initial_pos
    initial_orn = env.task.initial_orn

    print("111111111",env.task.target_pos)


    target_pose = env.task.target_pos


    

    env.task.reset_scene(env)
    env.task.floor_num = 0
    env.task.reset_agent(env)

    print("222222222",env.task.target_pos,initial_pos,initial_orn)
    ...
    return env, state , initial_pos, initial_orn, target_pose


def reset_new_1(env,initial_pos,initial_orn,target_pos):
    ...
    print("env.task.initial_pos",env.task.initial_pos)

    env.task.initial_pos = initial_pos
    env.task.initial_orn = initial_orn
...
def main():
    import copy 
    env_name = "random_end_effort_ridm"


    has_continuous_action_space = True  # continuous action space; else discrete

    train_times = 500
    config_filename = "./gibson_config.yaml"
    env = iGibsonEnv(config_file=config_filename, mode='headless')

    state_dim = 8

    action_dim = env.action_space.shape[0]
    env,state,initial_pos,initial_orn,target_pos = reset_new_0(env)
    print("state_0",state['task_obs'])

    action = env.action_space.sample()

    state, reward, done, _ = env.step(action)   
    print("state_1",state['task_obs'],reward)



    env, state  = reset_new_1(env,initial_pos,initial_orn,target_pos)
    print("state_0",state['task_obs'])
    state, reward, done, _ = env.step(action)
    print("state_1",state['task_obs'],reward) 
      
    # state = env.reset()
    # state_use = state['task_obs']
    # print(state_use)
    # use_env = copy.deepcopy(env)



    # state = state['task_obs']



    # RIDM_train(env, expert_trajectory,joint_positions ,
    #     ee_target_pose, train_times)
