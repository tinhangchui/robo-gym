...
class EndEffectorPositioningUR(URBaseEnv):
    ...
    def _get_observation_space(self) -> gym.spaces.Box:
        ...
        # Coke can position range
        coke_can_range = np.full(3, np.inf)
        # Definition of environment observation_space
        max_obs = np.concatenate((target_range, max_joint_positions, max_joint_velocities, coke_can_range))
        min_obs = np.concatenate((-target_range, min_joint_positions, min_joint_velocities, -coke_can_range))
    ...
    def _robot_server_state_to_env_state(self, rs_state) -> np.array:
        ...
        # Coke can positions
        coke_can_positions = []
        coke_can_keys = ['coke_can_x', 'coke_can_y', 'coke_can_z']
        for pos in coke_can_keys:
            coke_can_positions.append(rs_state[pos])
        coke_can_positions = np.array(coke_can_positions)


        # Compose environment state
        state = np.concatenate((target_polar, joint_positions, joint_velocities, coke_can_positions))
    ...
    def step(self, action) -> Tuple[np.array, float, bool, dict]:
        ...
        state = self.client.get_state_msg().state_dict
...
class WayPointSim(EndEffectorPositioningUR, Simulation):
...
class WayPointRobot(EndEffectorPositioningUR):
    real_robot = True
